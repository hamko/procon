...
int main(void) {
    ...
    ll n; cin >> n;
    vll a(n); rep(i, a.size()) {
        cin >> a[i];
        a[i]--;
    }
    vector<P> used(n, P(-1, 0));

    Mod ret = 1;
    ll others = n;
    rep(i, n) {
        if (used[i].fi != -1) continue;
        ll v = i;
        ll counter = 1;
        ll last_counter = -1;
        bool is_cycle = 0;
        while (1) {
            if (used[v].fi != -1 && used[v].fi != i) {
//                cout << v << " " << used[v]<<endl;
                break;
            }
            if (used[v].fi == i && used[v].se != 0) {
...
//        cout << used<<endl;
        ll cycle = last_counter - used[v].se;
        ret *= ((Mod(2) ^ cycle) - Mod(2));
//        cout << ret << endl;
