...
int main(void) {
    ...
    ll q; cin >> q;
    vll aa(q),bb(q),cc(q);rep(i,q)cin>>aa[i]>>bb[i]>>cc[i];

    vector<unordered_map<ll, ll>> hoge = factorizeRange(1e5+10);
    rep(i, facts.size()) if (i) {
        for (auto p : hoge[i]) if (p.fi > 1) {
            facts[i].pb(p.fi);
        }
    }
    rep(_, q) {
        ll a=aa[_], b=bb[_], c = cc[_];
        set<vll> memo, now, next;
        now.insert({a, b, c});
        memo.insert({a, b, c});
        while (now.size()) {
            next.clear();
