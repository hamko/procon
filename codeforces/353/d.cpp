...
int main(void) {
    ...
    rep(i, n) index[a[i]] = i;
    set<ll> s;
    set<ll, greater<ll>> sr;
    vvll tree(n, vll(2, -1));

    ll m = a[0];
    s.insert(a[0]), sr.insert(a[0]);
    repi(i, 1, n) {
        auto low = sr.lower_bound(a[i]);
        auto high = s.lower_bound(a[i]);
        if (a[i] < m) {
            tree[index[m]][0] = a[i];
            cout << m << " ";
        } else if (low != sr.end() && tree[index[*low]][1] == -1) {
            tree[index[*low]][1] = a[i];
            cout << *low << " ";
        } else if (low != sr.end() && tree[index[*low]][1] != -1) {
            tree[index[*high]][0] = a[i];
            cout << *high << " ";
        } else {
            cout << "HIT" << endl;
            assert(0);
