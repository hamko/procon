...
int main(void) {
    ll n, x; cin >> n >> x;
    vector<vector<tuple<ll, ll, ll>>> g(200010);
    rep(i, n) {
        ll l, r, c; cin >> l >> r >> c;
        g[r-l+1].pb(mt(l, r, c));
    }
    ...
    repi(i, 1, x) {
        ...
        vector<P> v;
        rep(i, b.size()) {
            v.pb(P(get<0>(b[i]), get<2>(b[i])));
        }
        sort(all(v));
        for (int i = v.size()-2; i >= 0; i--) {
            v[i].se = min(v[i+1].se, v[i].se);
        }
        rep(i, a.size()) {
            ll y = get<1>(a[i]);
            // vのyより大きいfiを持っているものを探す
            auto low = lower_bound(all(v), P(y, INF));
            if (low == v.end()) continue;
            chmin(ret, get<2>(a[i]) + low->se);
        }
    }
    if (ret == INF) {
        cout << -1 << endl;
