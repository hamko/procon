...
int main(void) {
    ...
    rep(i, n) if (g[i].size()) {
        ...
        for (auto&& x : g[i]) {
            ...
            if (x.size() > 2) {
                for (auto&& y : g[x[0]]) if (y[0] == x.back()) {
                    y.clear();
                    y.pb(i);
                    y.pb(x[1]);
                    vll tmp;
    ...
    ll nn = id.size();
    vvll gg(nn, vll(nn, INF));
    rep(i, nn) gg[i][i] = 0;
    rep(i, n) {
        for (auto x : g[i]) if (x[0] != -1) {
            gg[id[i]][id[x[0]]] = x[1];
            gg[id[x[0]]][id[i]] = x[1];
        }
    }
    ...
    rep(_, q) {
        ll u, v; cin >> u >> v; u--, v--;
        ll ret = INF;
        for (auto x : moyori[u]) for (auto y : moyori[v]) {
            chmin(ret, x.se + y.se + gg[id[x.fi]][id[y.fi]]);
        }
        cout << ret<< endl;
    }
