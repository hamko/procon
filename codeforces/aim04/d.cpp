...
typedef struct hoge_ {
    ...
    ll id = 0;
    hoge_() {}
    hoge_(ll v_, ll next_, ll id_) {v=v_,next=next_, id=id_;}
    bool operator<( const hoge_& right ) const {
        return v < right.v;
    }
} query;
int main(void) {
    ...
    // x以下の最大のvをもつidを取得
    query x_q; x_q.v = x;
    auto it = lower_bound(all(q), x_q, [](query l, query r) -> bool {return l.v < r.v;});
    if (it->v == x) {
        cout << "! " << it->id << endl;
        return 0;
    }
    it--;
//    cout << it->v << " " << it->id << " " << it->next << endl;
    ...
    rep(i, 900) {
        ...
        cout << "? " << t.next << endl; fflush(stdout);
        query y;
        y.id = t.next;
        cin >> y.v >> y.next;
        t = y;
