...
int main(void) {
    ...
    ll px = 0, py = 0;
    rep(i,n) {
        double p1x = px + x[i], p1y = py + y[i];
        double p2x = px - x[i], p2y = py - y[i];
...
//    cout << px << endl;
    while ((double)px*(double)px+(double)py*(double)py > 1.49e6*1.49e6) {
        ll i=((ll)rand()*(ll)rand())%n;
//        cout << i << " " << ret << " " << px << " " << py << endl;
        px -= ret[i] * x[i];
        py -= ret[i] * y[i];
        double p1x = px + x[i], p1y = py + y[i];
        double p2x = px - x[i], p2y = py - y[i];
