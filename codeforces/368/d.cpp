...
vll ret; // num
vll ans; 
void dfs(ll v) { // state: board[v], ret[v]
    /*
    cout << "##########" << endl;
    cout << v << "#v" << endl;
    cout << ret[v] << "#ret" << endl;
    if (v)
        cout << qtype[v-1] << " " << a[v-1] << " " << b[v-1] << "#qtype" << endl;
    rep(i, h) {
...
int main(void) {
    ...
    cin >>q;
    g = vvll(q+1);
    ret = vll(q+1), ans = vll(q+1);
    board = vvll(h, vll(w, 0));
    qtype = vll(q, -1), a = vll(q, -1), b = vll(q, -1);
