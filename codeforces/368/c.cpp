...
//        cout << -1 << endl;
    ...
    getPrimeFactorizationList(n, divisors_list);
    ll odd = -1;
    rep(i, divisors_list.size()) {
        if (divisors_list[i] % 2 == 1ll) {
            odd = divisors_list[i];
            break;
        }
    }
    ...
    if (odd < 0) {
//        cout << 3ll * n / 4ll << " " << 5ll * n / 4ll << endl;
    } else {
//        ll a = (odd - 1ll) / 2ll;
//        cout << 2ll * a * (a + 1ll) * (n / odd) << " " << (a * a + (a+1ll)*(a+1ll) )* (n / odd)<< endl;
    }
