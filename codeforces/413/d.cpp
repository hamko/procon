...
int main(void) {
    ...
    ll maxn = min<ll>({34, (ll)s.size()});

    // dp[i][j] = s[0:i)をかけてj以上にするのに最小の個数
    ll MAX = 200010;
    vvll dp(maxn+1, vll(MAX+1, INF));
    dp[0][1] = 0;
    rep(i, maxn) {
        rep(j, MAX) {
            ...
            if (j * s[i] < MAX)
                ...
                chmin(dp[i+1][dp[i+1].size()-1], dp[i][j] + 1);
    ...
    rep(__, 2) { swap(a, b);
        rep(n, maxn+1) {
            rep(_, 2) { swap(h, w), swap(a, b);
                ll need_h = (a+h-1)/h;
                ll h_j = INF, h_num = INF;
                repi(j, need_h, dp[n].size()) if (dp[n][j] != INF) {
                    h_j = j;
                    h_num = dp[n][j];
                    break;
                }
