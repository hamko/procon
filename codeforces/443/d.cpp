...
int main(void) {
    ...
    rep(i, n) {
        if (prev != a[i]) {
            while (c.size() > 1 && c.back().fi == c[c.size()-2].fi) {
                c[c.size()-2].se += c.back().se;
                c.pop_back();
            }
            if (c.size() && c.back().se % k == 0) {
                ret += c.back().se * m;
                c.pop_back();
            }
            c.pb(P(a[i], 1));
    ...
    auto c_org = c;
    while (c.size() > 1) {
        if (c.back().fi == c.front().fi && (c.front().se + c.back().se) % k == 0) {
            ret += (c.front().se + c.back().se) * (m - 1);
            l.pb(c.front());
            r.push_front(c.back());
            c.pop_back();
            c.pop_front();
    ...
    if (c.size() == 1) {
        ...
        if ((c[0].se *m) % k == 0) {
            while (l.size() && r.size()) {
                if ((l.back().se + r.front().se) % k == 0) {
                    ret += (l.back().se + r.back().se);
                    l.pop_back();
                    r.pop_front();
                } else {
                    break;
