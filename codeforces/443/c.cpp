...
int main(void) {
    ll n; cin >> n;
    vector<char> s(n); vll a(n);
    rep(i, n) {
        cin >> s[i] >> a[i];
    }
    function<ll(ll)> f = [&](ll v) -> ll{
        ll ret = v;
        rep(i, n) {
            if (s[i] == '|') {
                ret |= a[i];
...
//        cout << bits_to_string(v, 10) << " " << bits_to_string(f[v], 10) << endl;
    }
    */
    ll andop = (1ll<<10)-1, orop = 0, xorop = 0;
    rep(i, 10) {
        ll x0 = !!(f(0) & (1ll<<i)), x1 = !!(f(1ll<<i) & (1ll<<i));
//        cout << i << " : " << x0 << " " << x1 << endl;
        if (x0 == 0 && x1 == 0) {
            andop = ~andop;
            andop |= 1ll << i;
    ...
    cout << "&" << " " << bits_to_string(andop, 10) << endl;
    cout << "|" << " " << bits_to_string(orop, 10) << endl;
    cout << "^" << " " << bits_to_string(xorop, 10) << endl;
    */
