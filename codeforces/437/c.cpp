...
ll solve(ll n, ll k, vll s, vll a, vll b) {
    ll sum = 0;
    ll asum = 0;
    rep(i, n) {
        sum += s[i];
    ...
    vector<tuple<ll, ll, ll, ll>> ds(n);
    rep(i, n){
        ds[i] = mt(b[i] - a[i], s[i], a[i], b[i]);
    }
    sort(all(ds));
    ...
    rep(i, n){
        if (get<0>(ds[i]) > 0) 
            plus_sum += get<1>(ds[i]);
        else if (get<0>(ds[i]) < 0)
            minus_sum += get<1>(ds[i]);
    }
...
//        cout << i << " "<< tmp<< " " << cand << endl;
    }
    rep(i, n) {
//        cout << ds << endl;
        cand += get<1>(ds[i]) * get<2>(ds[i]);
    }
//    cout << cand << endl;
    chmax(ret, cand);
...
int main(void) {
    ...
    ll ret = 0;
    chmax(ret, solve(n, k, s, a, b));
    chmax(ret, solve(n, k, s, b, a));
    cout << ret << endl;
    return 0;
}
