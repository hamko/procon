...
int dp[11][1000001], mark[100001];
int main(void) {
    rep(i, 11) dp[i][0] = 1;
    rep(i, 1000001) dp[0][i] = 0;
    pair<int, int> mp1[100001];
    ...
    for (int i = 1; i < 11; i++) {
        for (int j = 1; j < 1000001; j++) {
            ...
            if (dp[i][j] >= 100001)
                break;
            if (!mark[dp[i][j]])
                mp1[dp[i][j]] = mp(i, j), mark[dp[i][j]]=1;
        }
    }
    ll a; cin >> a;
    cout << mp1[a].se << " " << mp1[a].first << endl;
    for (int i = 1; i <= mp1[a].first; i++) cout << i <<" ";
    return 0;
}
