...
int main(void) {
    int n; input(n);
    vector<int> a(n); rep(i, n) input(a[i]);
    SegmentTreeMin<int> smin(n);
    SegmentTreeMax<int> smax(n);
    vector<set<int>> val(1000010);
    rep(i, n) {
        val[a[i]].insert(i);
        smin.update(i, a[i]);
        smax.update(i, a[i]);
    ...
    rep(i, n) {
        if (!num) break;
        rep(j, n) { cout << smin.query(j, j+1) << " "; } cout << endl;
        rep(j, n) { cout << smax.query(j, j+1) << " "; } cout << endl;
        // 後から
        int v = smin.query(0, n);
        cout << v << "#v"<<endl;
        cout << val[v] << "#valv"<<endl;
        if (v == INF) { // 全部撮り終えた
            break;
        }
        int vi = *val[v].rbegin();
        cout <<vi<<"#vi"<<endl;
        val[v].erase(*val[v].rbegin()), num--; 
        ...
        // 前から
        cout << vi+1 << " " << n << "#querymax" << endl;
        int u = smax.query(vi+1, n);
        cout << u << "#u"<<endl;
        if (u == -INF) { // vに対応する場所がもう無い
            continue;
        }
        cout << val[u] << "#valu"<<endl;
        if (v >= u) { // 減少列になってしまっている
            continue;
        }
        auto uit =val[u].upper_bound(vi); 
        int ui = *uit;
        vll tmp;
        repi(j, vi, ui+1) if (smax.query(j, j+1) != -INF) { // うまいことスキップしないと TODO
            tmp.pb(j);
            // 削除は？ TODO
            val[a[j]].erase(val[a[j]].lower_bound(j)), num--; 
            smin.update(j, INF); smax.update(j, -INF);
        }
        rep(j, tmp.size()-1)
            if (tmp[i] < tmp[i+1]) 
                ret += tmp[i+1] - tmp[i];
