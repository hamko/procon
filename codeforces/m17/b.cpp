...
ll solve(void) {
    SegmentTreeMin<ll> smin(n);
    SegmentTreeMax<ll> smax(n);
    rep(i, n) {
        smin.update(i, a[i]);
        smax.update(i, a[i]);
    }
    ...
    rep(j, n) {
        ll tmp = q * a[j];
        if (p > 0) 
            tmp += p * smax.query(0, j+1);
        else 
            tmp += p * smin.query(0, j+1);
        if (r > 0) 
            tmp += r * smax.query(j, n);
        else 
            tmp += r * smin.query(j, n);
...
ll brutal(void) {
    ll ret = -INF;
    rep(i, n) rep(j, n) rep(h, n) {
        if (i <= j && j <= h) {
            chmax(ret, p * a[i] + q * a[j] + r * a[h]);
        }
    }
    return ret;
}
