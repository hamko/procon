...
int main(void) {
    ...
    while (zeropos.size() > 1) {
        cout << 1 << " " << zeropos.back()+1 << " " << zeropos[0]+1 << endl; rem--;
        a[zeropos.back()] = INF;
        zeropos.pop_back();
    }
    ll z = -1;
    ...
    if (minus_num > 0 && minus_num % 2 == 1) {
        ll mi = -1;
        rep(i, n) if (a[i] != INF && a[i] < 0) {
            if (mi == -1 || a[mi] < a[i]) {
                mi = i;
            }
        }
        if (z != -1) {
            cout << 1 << " " << mi + 1 << " " << z + 1 << endl; rem--;
            a[mi] = INF;
        } else {
            cout << 2 << " " << mi + 1 << endl; rem--;
            a[mi] = INF;
        }
        minus_num--;
    }

    if (minus_num == 0 && plus_num == 0) {
        assert(rem == 0); return 0;
    }
    ...
    rep(i, valid.size() - 1) {
        cout << 1 << " " << valid[i+1] + 1 << " " << valid[0] + 1 << endl; rem--;
    }
