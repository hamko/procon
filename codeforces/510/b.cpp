...
int main(void) {
    ll n; cin >> n;
    vll p(n); vector<string> s(n);
    ll a=INF,b=INF,c=INF,ab=INF,bc=INF,ac=INF,abc=INF;
    rep(i, n) {
        cin >> p[i] >> s[i];
        sort(all(s[i]));
    ...
    memo.pb(mp(abc, "ABC"));
//    cout << memo << endl;
    rep(mask, 1ll<<(memo.size())) {
        set<char> cc;
        ll tmp = 0;
        rep(i, memo.size()) if (mask & (1ll << i)) {
            tmp += memo[i].fi;
            for (auto c : memo[i].se) {
                cc.insert(c);
            }
        }
//        cout << cc << " " << tmp << endl;
