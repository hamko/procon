...
#define mp make_pair
template<class T1, class T2> T1 chmin(T1 &a, T2 b) { return b < a && (a = b, true); }
template<class T1, class T2> T1 chmax(T1 &a, T2 b) { return a < b && (a = b, true); }
...
int main(void) {
    ...
    vvll g(n);
    rep(i, n) {
        // iより大きく、かつiを包含するものの中で最小の円
        ll ret = -1;
        rep(j, n) if (rr[i] < rr[j] && sqrt(pow(x[i]-x[j],2)+pow(y[i]-y[j],2)) + (double)rr[i] < (double)rr[j] + 1e-9) {
            if (rr[ret] > rr[j]) {
                ret = j;
            }
        }
...
//    cout << g << endl;
    ...
    rep(i, 1100) rep(j, 2) rep(h, 2) dp[i][j][h] = -INF;
    function<double(ll, ll, ll)> dfs = [&](ll v, ll l, ll r) {
        double& ret = dp[v][l][r];
        if (ret != -INF) return ret;
        double s = (l?-1:1)*pi*rr[v]*rr[v], t = (r?-1:1)*pi*rr[v]*rr[v];
        for (auto c : g[v]) {
            s += dfs(c,l^1,r);
            t += dfs(c,l,r^1);
        }
    ...
    /*
    cout << roots << endl;
    rep(i, n)rep(j,2)rep(h,2) if(dp[i][j][h]!=INF){
