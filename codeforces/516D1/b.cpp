...
int main(void) {
    ...
    rep(i, n) rep(j, m) if (b[i][j] != '*') {
        ...
        rep(d, 2) {
            ll ii = i + di[d];
            ll jj = j + dj[d];
            if (0 <= ii && ii < n && 0 <= jj && jj < m && b[ii][jj] != '*') {
                uf.unite(i*m+j, ii*m+jj);
            }
        }
    }
//    cout << uf.getUnionList() << endl;
//    cout << uf.root(10) << endl;
    ...
    rep(i, n) rep(j, m) if (b[i][j] != '*') {
        ...
        rep(d, 2) {
            ...
            ll jj = j + dj[d];
            if (0 <= ii && ii < n && 0 <= jj && jj < m && b[ii][jj] != '*') {
                g[uf.root(i*m+j)].pb(uf.root(ii*m+jj));
            }
        }
    }
    ...
    */
    ...
    ll sij = uf.root(si*m+sj);
    vector<P> dist(n*m, P(INF, INF));
    queue<ll> q;
    q.push(sij);
    dist[sij] = P(l, r);
    while (q.size()) {
        ...
        ll i = croot / m;
        ll j = croot % m;
//        cout << mt(i, j) << "#state" << endl;

        q.pop();
        for (auto next_croot : g[croot]) if (dist[next_croot].fi == INF) {
            ll next_i = next_croot / m;
            ll next_j = next_croot % m;
//        cout << mt(next_i, next_j) << "#try" << endl;
            if (j < next_j) {
                if (dist[croot].se > 0) {
                    dist[next_croot] = dist[croot];
                    dist[next_croot].se--;
                    q.push(next_croot);
                }
            } else if (j > next_j) {
    ...
    ll ret = 0;
    rep(i, n) rep(j, m) if (dist[i*m+j].fi != INF) {
//        cout << mt(i, j) << i*m+j<< " " << dist[i*m+j] << endl;
        ret += uf.size(i*m+j);
    }
    cout << ret << endl;
