...
int main(void) {
    ...
    rep(i, n) {
        rep(j, m) {
            as[i][j+1] = as[i][j] + (a[i][j] - '0');
        }
    }
//    cout << "$$$" << endl;
    vvll bt(n, vll(m+1, INF));
    rep(i, n) {
        rep(l, m) repi(r, l, m+1) {
            chmin(bt[i][r-l], as[i][l] + as[i][m] - as[i][r]);
        }
    }
    vvll b(n, vll(550, 0));
    rep(i, n) {
...
//    cout << "$$$" << endl;
    repi(i, 1, n+1) {
        for (ll j = 501; j >= 0; j--) {
            rep(k, j+1) {
                chmin(dp[i][j], dp[i-1][j-k] + b[i-1][k]);
            }
        }
        /*
        rep(j, m+1) if (dp[i][j] != INF) {
