...
#define INF (ll)1e18
#define mo  (ll)(1e9+7)
ll score(vector<string> b) {
    ll n = b.size();
    ll ret = INF;

    {
        ll counter = 0;
        rep(i, n) rep(j, n) counter += (b[i][j] == ('0'+((i+j+0)%2)));
        chmin(ret, counter);
    }
    {
        ll counter = 0;
        rep(i, n) rep(j, n) counter += (b[i][j] == ('0'+((i+j+1)%2)));
        chmin(ret, counter);
    }
