...
class Solution {
    public:
        ...
        ll dfs(ll l, ll r, ll k) {
            ...
            ll ret = -INF;
            chmax(ret, dfs(l, r-1, 0) + (k+1)*(k+1));
            for (ll x = l; x+1 <= r; x++) {
                chmax(ret, dfs(l, x, 0) + dfs(x+1, r, k));
            }
            for (ll x = l; x+1 <= r; x++) if (a[x] == a[r]) {
                chmax(ret, dfs(l, x, k+1) + dfs(x+1, r-1, 0));
            }
            return dp[l][r][k] = ret;
        }
...
//            cout << dfs(0, n-1, 0) << endl;
           /*
            rep(i, 101) rep(j, 101) rep(h, 101) if (dp[i][j][h] != -1){
