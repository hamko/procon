...
class ArithmeticSequenceDiv1 {
    public:
        int brutal(vector<int> x) {
            ...
            rep(i, sz) {
                repi(j,i+1,sz) {
                    int diff = 0;
                    if (x[j] >= x[i]) {
                        diff = ceil((x[j]-x[i])/(j-i));
                    } else {
                        diff = floor((x[j]-x[i])/(j-i));
                    }
                    int cnt=0;
                    rep(k,sz){
                        cnt+=abs(x[k]-(x[i]+diff*(k-i)));
                    }
                    ans=min(ans,cnt);
                    cnt=0;
                    rep(k,sz){
                        cnt+=abs(x[k]-(x[j]+diff*(k-j)));
                    }
                    ans=min(ans,cnt);
                }
            } 
        ...
        int findMinCost(vector <int> x) {
            ll mm = 0;
            while (1) {
                if (mm % 100==0) cout << mm << endl;
                mm++;
                vector<int> a(1+rand()%100);
                rep(i, a.size()) {
                    a[i] = 1+rand()%100;
                }
                ll s = brutal(a);
...
bool KawigiEdit_RunTest(int testNum, vector <int> p0, bool hasAnswer, int p1) {
	...
	cout << "}";
	cout << "]" << endl;
	ArithmeticSequenceDiv1 *obj;
	int answer;
	obj = new ArithmeticSequenceDiv1();
	clock_t startTime = clock();
	answer = obj->findMinCost(p0);
