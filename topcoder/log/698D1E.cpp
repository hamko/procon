...
class RepeatString {
    public:
        ll dist(string s, string t) {
            ll sn = s.length();
            ll tn = t.length();
            vvll dp(sn+2, vll(tn+2, INF));
            rep(i, sn+1) dp[i][0] = i;
            rep(i, tn+1) dp[0][i] = i;
            rep(i, sn+1) rep(j, tn+1) if (dp[i][j] != INF) {
                ll v = dp[i][j];
                if (s[i] == t[j]) 
                    chmin(dp[i+1][j+1], v);
                if (s[i] != t[j])
                    chmin(dp[i+1][j+1], v + 1);
                chmin(dp[i+1][j], v + 1);
                chmin(dp[i][j+1], v + 1);
            }
            return dp[sn][tn];
        ...
        int minimalModify(string s) {
            ll ret = INF;
            rep(len, s.length()+1) {
                ll d = dist(s.substr(0, len), s.substr(len, s.length()-len));
                chmin(ret, d);
            }
            return ret;
        }
...
int main() {
	...
	// ----- test 5 -----
	disabled = false;
	p0 = "pkafkbeabccfjejkdgkaatcedaocgmecaapakfvbfgefr";
	p1 = 20;
