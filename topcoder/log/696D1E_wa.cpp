    ...
    public:
        ...
        int countfee(vector <int> x, vector <int> y) {
            ...
            rep(i, n) {
                ...
                vll dp(1<<s.size(), INF);
                dp[0] = 0;
                rep(maski, 1 << s.size()) { // もらうDP
                    ll& val = dp[maski];
                    rep(j, s.size()) if (maski & (1 << j)) {
                        ll prev_maski = maski & (~(1ll << j)); // j bit目を0にする
                        // prev_maskiの状態から、jを塗るときのコストは？
                        ll cost = 0;
                        rep(h, s.size()) if (prev_maski & (1ll << h)) {
                            cost += g[s[h]][s[j]];
                        }
                        chmin(val, dp[prev_maski] + cost);
                    }
                }
                ret += dp.back();
