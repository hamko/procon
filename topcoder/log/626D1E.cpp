...
class FixedDiceGameDiv1 {
    public:
        double getExpectation(int a, int b, int c, int d) {
            if (a * b <= c) return -1;
            vector<vector<double>> D(a+1, vector<double>(N)), E(c+1, vector<double>(N));
            D[0][0] = E[0][0] = 1;
            rep(i, a) rep(j, N) rep(k, b) D[i+1][j+k+1] += D[i][j];
            rep(i, c) rep(j, N) rep(k, d) E[i+1][j+k+1] += E[i][j];
            /*
            cout << D[a] << endl;
            cout << E[c] << endl;
            */

            vector<double> f(N); // f[x] = A=x>Bである場合の数
            rep(x, N) rep(y, x) f[x] += D[a][x] * E[c][y];
//            cout << f << endl;
            double s = 0;
            rep(x, N) rep(y, x) s += D[a][x] * E[c][y];
