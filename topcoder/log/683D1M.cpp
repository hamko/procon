...
class GCDLCM2 {
    public:
        int getMaximalSum(vector <int> s, vector <int> d, vector <int> c) {
//            constructPrime(1e7+10);
            constructPrime(1e5+10);
            vll a; rep(i, s.size()) rep(j, c[i]) a.pb(s[i] + d[i] * j);
//            cout << a << endl;
            ll n = a.size();
            map<ll, vll> p;
            rep(i, n) {
...
// 
// 
// You are given the vector <int>s start, d, and cnt, each with the same number of elements.
...
// Returns:int
// Method signature:int getMaximalSum(vector <int> start, vector <int> d, vector <int> cnt)
// 
// 
// NOTES
