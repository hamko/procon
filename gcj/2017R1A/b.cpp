...
int main(void) {
    ll _; cin >> _;
    rep(__, _) {
//        cout << __ << "#testcase" << endl;
        ll n, p; cin >> n >> p;
        vld R(n); cin >> R;
        vector<vld> q(n, vld(p));
        rep(i, n) rep(j, p) {
            cin >> q[i][j];
        }
        ll maxret = 0;
...
//            cout << q << endl;
            ...
            rep(j, p) {
                ...
                double l = -INF, r = INF;
                rep(i, n) {
                    chmax(l, q[i][j] / 1.1 / R[i]);
                    chmin(r, q[i][j] / 0.9 / R[i]);
                }
                if (ll(ceil(l-EPS)) <= ll(floor(r+EPS))) {
                    ret++;
                }
#else 
                ll ok = 0;
                ...
                rep(k, 1e6) {
                    ...
                    ll ff = 1;
                    rep(i, n) {
//                        cout << k<<  " " << i << " " << n << " " << 0.9*k*R[i] << " " << q[i][j] << " "  << 1.1*k*R[i] << endl;
                        if (!(0.9*k*R[i]-EPS<=q[i][j] && q[i][j] <= 1.1*k*R[i]+EPS)) {
                            f = 0;
                            break;
                        }
                    }
            ...
            chmax(maxret, ret);
        } while (q.size() == 2 && next_permutation(all(q[0])));

        cout << "Case #" << __+1 << ": " << maxret << endl;
    }
