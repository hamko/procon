    ...
    rep(__, _) {
        ...
        function<ll(ll, ll, ll, ll)> 
            ...
            repi(i, i1, i2+1) repi(j, j1, j2+1) if (ret[i][j] != '?')
                counter[ret[i][j]]++;
            return accumulate(all(counter), 0ll);
        };
        
        for (auto pos : memo) {
            char x = pos.fi; ll i = pos.se.fi, j = pos.se.se;
            ll i1 = 0, i2 = 0, j1 = 0, j2 = 0;
            while (countAlphabets(i-(i1+1), i+i2, j-j1, j+j2) == 1) i1++;
            while (countAlphabets(i-i1, i+(i2+1), j-j1, j+j2) == 1) i2++;
            while (countAlphabets(i-i1, i+i2, j-(j1+1), j+j2) == 1) j1++;
            while (countAlphabets(i-i1, i+i2, j-j1, j+(j2+1)) == 1) j2++;
            repi(ii, i-i1, i+i2+1) repi(jj, j-j1, j+j2+1)
                ret[ii][jj] = x;
        }
