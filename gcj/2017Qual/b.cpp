...
ll solveLarge(ll _, ll n) {
    assert(is_ass(1011) == 0);
    assert(is_ass(561) == 0);
    assert(is_ass(1111) == 1);
    string s;
    vll ret;
    ll d = 0;
    ll nn = n;
...
//            cout << ret[i] << endl;
    ...
    cout << "Case #" << _+1 << ": " << max_ret << endl;
    return max_ret;
}

ll solveSmall(ll _, ll n) {
    while (n) {
        ll t = n;
        string s;
        while (t) {
...
int main(void) {
    ...
    rep(_, q) {
        ...
        /*
        if (n < 100000) 
            assert(solveSmall(_, n) == solveLarge(_, n));
            */
    }
