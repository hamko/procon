...
template <typename T> ostream &operator<<(ostream &o, const vector<T> &v) { o << '['; rep(i, v.size()) o << (v[i] == INF ? "inf" : (v[i] == -INF ? "-inf" : to_string(v[i]))) << (i != v.size()-1 ? ", " : ""); o << "]";  return o; }
...
// 関数fが単調mongeかを判定する。
// これがmongeならば、以下がmongeになる
// dp[i][j] = min(dp[i-1][0, i)) + w(i, j)
//
// O(n^4)
void diagonose_MonotoneMongeFunction(function<ll(ll, ll)> f, ll n) {
    ll faf_monge = 0;
    rep(a, n) repi(b, a, n) repi(c, b, n) repi(d, c, n) 
        faf_monge &= f(a, b) + f(b, d) <= f(a, d) + f(b, c);
    if (faf_monge) 
        cout << "Monge!" << endl;
    else
        cout << "Not monge" << endl;
...
// DPにおいて、dp[i][j]を実現する最小の遷移kの性質を判定する。
// 判定される性質は3つ
// (1) 単調性
// (2) monge
//
// O(n m)
void diagonose_DP(vvll A) {
    assert(A.size() && A[0].size());
    ll n = A.size(), m = A[0].size();

    ll faf;
    
    // dp[i][j] = min_{k < j} {dp[i - 1][k] + f(k, j)}
    //
    // A[i][j] <= A[i][j+1]
    // fがmonge（単調性は要らないはず）
    // これが満たされていれば分割統治法
    ...
    rep(i, n) rep(j, m-1) {
        if (A[i][j] >= 0 && A[i][j+1] >= 0 && !(A[i][j] <= A[i][j+1])) {
            cout << i << " " << j << " : " << A[i][j] << " " << A[i][j+1] << "#Counterexample" << endl;
            faf = 0;
            break;
        }
    }
    if (faf) 
        cout << "Divide and Conquer applicable!" << endl;
    else
        cout << "Divide and Conquer not applicable" << endl;

    // dp[i][j] = min_{i<k<j} {dp[i][k] + dp[k][j]} + f(i, j)
    //
    // A[i-1][j] <= A[i][j] <= a[i][j+1]
    // fがmonge & monotone
    // これが満たされていればKnuth's optimizationができる
    // 重要！n=mじゃないと無理。そうじゃないと結局O(n^3)から落ちなくて悲しい気持ちになる
    faf = 1;
    rep(i, n) rep(j, m-1)
        if (A[i][j] >= 0 && A[i][j+1] >= 0 && !(A[i][j] <= A[i][j+1])) {
            cout << i << " " << j << " : " << A[i][j] << " " << A[i][j+1] << "#Counterexample" << endl;
    ...
    rep(i, n-1) rep(j, m)
        if (A[i][j] >= 0 && A[i+1][j] >= 0 && !(A[i][j] <= A[i+1][j])) {
            cout << i << " " << j << " : " << A[i][j] << " " << A[i+1][j] << "#Counterexample" << endl;
            faf = 0;
            break;
        }
    if (faf) 
        if (n == m) 
            cout << "Knuth optimization applicable!" << endl;
        else 
            cout << "It satisfies Monge, but not applicable because n != m. Use Devide and Conquer!" << endl;
    else
        cout << "Knuth optimization not applicable" << endl;

}
// DPにおいて、dp[i][j]が全てのiについてdp[i]が凸かを判定する
//
// O(n m)
void diagonose_DP_convex(vvll dp) {
    assert(dp.size() && dp[0].size());
    ll n = dp.size(), m = dp[0].size();
    ll faf;
    faf = 1;
    rep(i, n) {
        ll counter = 0;
        ll prev = -1;
        rep(j, m-1) {
            bool b = (dp[i][j] <= dp[i][j+1]);
            counter += (prev != -1 && prev != b);
            prev =  b;
        }
        faf &= prev <= 1;
    }
    if (faf) 
        cout << "Convex optimization applicable!" << endl;
    else
        cout << "Convex optimization not applicable" << endl;
}

// CF 438 Div.2 F (TLE解)
// これがMongeではできないことをきちんと認識できます
ll a[100010];
int main(void) {
    ll n, m; cin >> n >> m;
    rep(i, n) cin >> a[i];
    wavelet<ll, 100010, 18> w(n, a);

    {
        vvll dp(m+1, vll(n+1, INF));
        dp[0][0] = 0;
        vvll A(m+1, vll(n+1, -INF)); 
        repi(j, 1, m+1) {
            rep(i, n+1) {
                ll s = 0;
                for (int k = i - 1; k >= 0; k--) {
                    s += w.freq(k+1, i, a[k], a[k]+1);
                    if (dp[j][i] > dp[j-1][k] + s) 
                        dp[j][i] = dp[j-1][k] + s, A[j][i] = k;
                }
            }
        }
        rep(j,m+1) cout << dp[j] << endl;
        rep(j,m+1) cout << A[j] << endl;
        cout << dp[m][n] << endl;
        diagonose_DP(A);
    }
    return 0;
