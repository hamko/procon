...
public:
    ...
    vector<bool> ret;
    SAT (int n_) : n(n_), g(n*2), g_rev(n*2) { }
    inline int node(int t) {
    ...
    void addClause(int t1, int t2) {
        ...
        g_rev[node(t1)].pb(node(-t2));
        g_rev[node(t2)].pb(node(-t1));
    }

    // 従属可能で1を出力
    bool isSAT(void) {
        // SCC
        vector<int> color(g.size(), -1); // color[i] == color[j]ならばiとjは同じ連結成分
        {
            vector<int> id;
    ...
    // 解ありの場合、x_i = ret[i]がSATを与える。
    // 解なしの場合、ret.size() == 0である
    vector<bool> getAnswer(void) { return ret; }
    void print_answer(void) {
        if (ret.size()) {
            cout << "SAT : "; rep(i, n) cout << ret[i] << " "; cout << endl;
        } else {
            cout << "UNSAT" << endl;
        }
    }
...
// sat.isSAT
//      1ならSAT, 0ならUNSAT
//      SATなら副作用でsolver.ret[i]にx_{i+1}の割当結果が得られる
//      1がtrueで0がfalseである
...
int main(void) {
    ...
    while (1) {
        ...
        vector<P> clauses;
        rep(i, m) {
            int s = (int)(1+rand()%n); if (rand()%2) s *= -1;
            int t = (int)(1+rand()%n); if (rand()%2) t *= -1;
            vector<int> tmp = {s, t};
            sat.addClause(tmp);
            totest.addClause(s, t);
            clauses.pb(P(s,t));
        }
        ll ret_right = !sat.solve(); // 3-SATはsolveもassignsも逆！！！！注意
        ll ret_wrong = totest.isSAT();
        if (ret_right != ret_wrong) {
            cout << n << " " << clauses.size() << endl;
            rep(i, m) {
                cout << clauses[i].fi << " " << clauses[i].se << endl;
            }
            cout << ret_wrong << endl;
            cout << ret_right << endl;
            return 0;
