...
// https://colopl2018-qual.contest.atcoder.jp/submissions/1845161
 
// 集めるDP
// dp[i][j] = min_{0<=k<i} DP[i-1][k] + cost(k, j)
//
// 最大化問題の場合は、cost(k, j)の正負を反転させればよい。
ll dp[5010][5010];
ll n, x;
vll t;
// cost(k, j)
ll cost(ll k, ll j) {
    return -min(x, t[j]-t[k]);
}
...
int main(void) {
    ...
    repi(i, 1, n+1) // i in [1, n]を集めるDPで更新
        rec(i, 0, n, 0, n); // j in [0, n)を更新。遷移kは[0, n)の中のどれかはまだわからない
