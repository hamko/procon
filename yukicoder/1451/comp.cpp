...
Double binary(Double a, Double b, Double t) {
    if (!a) {
        return expl(pow(t, 1.0l / b));
    }
    if (!b) {
...
Double w(Double a, Double b, Double t) {
    if (!a) {
        return exp(pow(t, 1.0l / b));
    }
    if (!b) {
        return pow(t, 1.0l / a);
    }
    ...
    return expl(1.0l * b / a * LambertW(1.0l * a / b * powl(t, 1.0l / b))); 
}
...
int main(void) {
    ...
    rep(i, m) {
        ...
        Double binary_ret = binary(a, b, t);
        Double w_ret = w(a, b, t);
        Double err = abs(binary_ret - w_ret);
        chmax(maxerr, err);
        if (err > 1e-9) {
            cout << "!!!!!!!!!!!!!!!!" << endl;
            exit(1);
