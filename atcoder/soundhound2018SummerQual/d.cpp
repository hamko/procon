...
ll n, m;
vll dy, ds;
void solve1(vector<vector<P>> & g, vll& dist, ll st) {
    dist.resize(n);
    rep(i, n) {
        dist[i] = INF;
    }
...
int main(void) {
    ll s,t; cin >> n>>m>>s>>t;s--,t--;
    vector<vector<P>> gy(n);
    vector<vector<P>> gs(n);
    rep(i, m) {
...
//    cout << dy << endl;
//    cout << ds << endl;
    SegmentTreeMin<ll> q(n);
    rep(i, n) {
        q.update(i, dy[i]+ds[i]);
    }
    rep(i, n) {
        cout << (ll)(1e15)-q.query(i, n) << endl;
        q.update(i, INF);
    }
