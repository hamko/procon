...
int main(void) {
    ...
    s = string(n, s[0]) + s + string(n, s[n-1]);
    int i = 0, j = 0;
    vll r(s.length());
    while (i < s.size()) {
    ...
    cout << n*(n+1)/2ll - accumulate(r.begin()+n, r.begin()+n*2ll, 0ll) << endl;
    */

    map<ll, ll> c; rep(i, n) c[s[i]]++;
    ll ret = 0;
    for (auto x : c) for (auto y : c) if (x < y) ret += x.se * y.se;
    cout << ret + 1 <<endl;
    return 0;
}
