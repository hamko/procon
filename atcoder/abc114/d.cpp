...
int main(void) {
    ...
    map<ll, ll> m;
    repi(i, 1, n+1) {
        for (auto y : factorize(i)) {
            m[y.fi]+=y.se;
        }
    }

    vll memo;
    for (auto x : m) memo.pb(x.se);
    sort(all(memo));
//    cout << memo << endl;
    ...
    // 3 5 5
    vll id(memo.size());
    id[id.size()-1] = id[id.size()-2] = id[id.size()-3] = 1;
    do {
        vll cands;
        rep(i, id.size()) if (id[i]) cands.pb(memo[i]);
//        cout << id << " " << cands << endl;
        if (cands[0] >= 2 && cands[1] >= 4 && cands[2] >= 4) ret++;
        if (cands[0] >= 4 && cands[1] >= 2 && cands[2] >= 4) ret++;
        if (cands[0] >= 4 && cands[1] >= 4 && cands[2] >= 2) ret++;
...
//        cout << id << endl;
        vll cands;
        rep(i, id.size()) if (id[i]) cands.pb(memo[i]);
        if (cands[0] >= 2 && cands[1] >= 24) ret++;
        if (cands[0] >= 24 && cands[1] >= 2) ret++;
        if (cands[0] >= 4 && cands[1] >= 14) ret++;
        if (cands[0] >= 14 && cands[1] >= 4) ret++;
    } while (next_permutation(all(id)));
