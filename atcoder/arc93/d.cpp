...
ll count(vvll b, ll c) {
    vvll col(n, vll(n, -1));
    function<void(ll, ll, ll)> dfs = [&](ll i, ll j, ll x) {
        if (i < 0 || j < 0 || i >= n || j >= n || col[i][j] != -1 || b[i][j] != c) {
            return;
        }
        col[i][j] = x;
        vll dx = {0, 1, 0, -1};
...
int main(void) {
    ...
    repi(i, 0, n) if (i % 2 == 0) rep(j, n-1) {
        b[i][j] = 1;
    }

    rep(i, n) rep(j, n-1) if (i % 6 == 0 && j > 1 && j % 2 == 1 && b[i][j] == 1) { if (!x) break; b[i][j] = 0; x--; }
    rep(i, n) rep(j, n-1) if (i % 6 == 3 && i > 1 && j > 1 && j % 2 == 1 && b[i][j] == 0) { if (!y) break; b[i][j] = 1; y--; }
