...
//    cout << brutal(s) << endl;
    ll n = s.length();
    vll as(n+1); rep(i, n) as[i+1] = as[i] + (s[i] == 'A');
    vll cs(n+1); rep(i, n) cs[i+1] = cs[i] + (s[i] == 'C');
    vll qs(n+1); rep(i, n) qs[i+1] = qs[i] + (s[i] == '?');
//    cout << qs << endl;
    mint ret = 0;
    rep(i, n) if (s[i] == 'B' || s[i] == '?') {
        ll s = qs[i] - qs[0];
        ll t = qs[n] - qs[i+1];
//        cout << mt(i, s, t) << " " << mint(as[i] - as[0  ]) << " " << cs[n] - cs[i+1] << endl;
        ret += 
            ((mint(3)^s)*mint(as[i] - as[0  ]) + mint(s) * (mint(3)^(s - 1))) *
            ((mint(3)^t)*mint(cs[n] - cs[i+1]) + mint(t) * (mint(3)^(t - 1)));
    }
    cout << ret << endl;
