...
int main(void) {
    ...
    rep(i, m) {
        ...
        ll u, v; cin >> u >> v;
        g[u].pb(v), g[v].pb(u);
        memo[P(u, v)] = memo[P(v, u)] = type;
    }

    vll ret(n, INF);
    vector<vector<ll>> min_b(n); // min_b[i][j] = 頂点iにいて、j回bを通っている時の最小コスト
    ...
    while (q.size()) {
        ...
        dist = tmp.dist, v = tmp.v, b = tmp.b; 
        chmin(ret[v], dist);

        for (int next_v : g[v]) {
            ll is_b = memo[P(v, next_v)];
            int next_dist = dist + b * is_b + 1;
            int next_b = b + is_b;

            if (next_b >  n) continue;

            // それ以前のbよりも、next_distが小さい必要
            ll ok = 1;
            rep(i, min<ll>(next_b+1, min_b[next_v].size())) 
                ok &= min_b[next_v][i] > next_dist;
            if (!ok) continue;

            while (min_b[next_v].size() <= next_b)
                min_b[next_v].pb(INF);
            chmin(min_b[next_v][next_b], next_dist);

            q.push({next_dist, next_v, next_b});
        }
    }
