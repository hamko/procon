...
int main(void) {
    ...
    rep(i, n) {
        cin >> a[i] >> b[i];
        g[a[i]].pb(b[i]);
        if (!memo.count(P(a[i], b[i]))) {
            memo[P(a[i], b[i])] = i;
        }
    }
    vector<P> dp(N, P(0, INF));
    // 集めるDP
    for (ll i = N-2; i >= 0; i--) {
        dp[i].fi = dp[i+1].fi;
        for (auto x : g[i]) {
            if (dp[i].fi < dp[x].fi + 1) {
                dp[i].fi = dp[x].fi + 1;
            }
        }
        for (auto x : g[i]) if (dp[i].fi == dp[x].fi + 1) {
            chmin(dp[i].se, memo[P(i, x)]);
        }
        if (dp[i].fi == dp[i+1].fi) {
            chmin(dp[i].se, dp[i+1].se);
        }
//        cout << i << " " << dp[i] << " " << g[i] << endl;
    }
    vll ret;
    ll i = 0;
    while (dp[i].se != INF) {
