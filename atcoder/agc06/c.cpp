...
int main(void) {
    ll n; cin >> n;
    vll x(n); cin >> x;
    vll d(n-1); rep(i, n-1) d[i] = x[i+1] - x[i];
    ll m, k; cin >> m >> k;
    vll ord(n-1);
    iota(all(ord), 0);
    vvll ordk2(100, vll(n-1));
    rep(i, m) {
        ll u; cin >> u;
        swap(ord[u-1], ord[u-2]);
    }

    ordk2[0] = ord;
    rep(i, ordk2.size()-1) {
        ordk2[i+1] = merge(ordk2[i], ordk2[i]);
    }
    vll ret(n-1); iota(all(ret), 0);
    ll cyc = 0;
    while (k) {
        if (k % 2 == 1) {
            ret = merge(ret, ordk2[cyc]);
        } 
        k /= 2;
        cyc++;
    }
