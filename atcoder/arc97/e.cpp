...
int main(void) {
    ...
    rep(i, 2010) rep(j, 2010) pos[i][j] = P(-1, -1);
    rep(i, n + 1) { // Bを取ってくる個数
        SegmentTreeSum<ll> st(2*n);;
        rep(j, i) {
            st.assign(b[j], 1);
        }
        rep(j, n + 1) {
            pos[i][j] = P(i < n ? b[i] - st.sum(0, b[i]) : -1, j < n ? w[j] - st.sum(0, w[j]) : -1);
            if (j < n) st.assign(w[j], 1);
        }
    }
    ...
    */
    ...
    rep(i, n+1) rep(j, n+1) if (dp[i][j] != INF) {
        chmin(dp[i+1][j], dp[i][j] + max<ll>(0, pos[i][j].fi));
        chmin(dp[i][j+1], dp[i][j] + max<ll>(0, pos[i][j].se));
    }
    cout << dp[n][n] << endl;
