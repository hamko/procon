...
//    cout << g << endl;
    ...
    while (sec() < 1.9) {
        ...
        rep(i, traj.size()) {
            ...
            if (f == 1) {
                b.back()[edges_rev[P(traj[i], i==traj.size()-1?u:traj[i+1])]] = 1;
//                cout << traj[i] << "  " << (i==traj.size()-1?u:traj[i+1]) << endl;
                sc += es[P(traj[i], i==traj.size()-1?u:traj[i+1])];
            }
        }
        score.pb(sc);
    }
