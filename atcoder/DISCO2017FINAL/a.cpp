...
bool isOK(ll x, ll y, ll r) {
    return x * x + y * y <= r * r;
}

ll d;
ll solve(ll r, ll b = 0) {
    ll n = 2*r;
    ll ret = 0;
//    vvll g(50, vll(50));
    repi(x, -n, n) repi(y, -n, n) {
        if (isOK(x*d+b,y*d+b,r)&&isOK(x*d+d+b,y*d+b,r)&&isOK(x*d+b,y*d+d+b,r)&&isOK(x*d+d+b,y*d+d+b,r)) {
//            g[x+10][y+10] = 1;
//            cout << b << " " << x << " " << y << endl;
            ret++;
        }
...
int main(void) {
    cin >> d;
    cout << (200 / d % 2 == 1 ? solve(100, d/2) : solve(100)) << " " << (300 / d % 2 == 1 ? solve(150, d/2) : solve(150)) << endl;
//    cout << solve(100, d/2) << endl;
//    cout << max(solve(100), solve(100, d/2)) << " " << max(solve(150), solve(150, d/2)) << endl;
    return 0;
}
