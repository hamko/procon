...
int main(void) {
    ...
    rep(mask, pow(n, n*k)) {
        ll tmp = mask;
        if (mask % 10000 == 0) cout << mask << " / " << (ll)pow(n, n*k) << endl;
        vll freq(n);
        rep(i, n) rep(j, k) {
            a[i][j] = tmp % n;
            freq[a[i][j]]++;
            if (freq[a[i][j]] > n) goto SKIP;
            tmp /= n;
        }
        if (check()) {
            cout << a << endl;
    ...
    vll size_vec = {10, 50, 100, 500, 1000, 2000, 5000}; 
    for (auto n : size_vec) {
        cout << n << "######################################" << endl;
        togasat::Solver sat(n);
        rep(i, n * n / 10) {
            vector<int> tmp = {(int)(1 + rand() % n), (int)(1 + rand() % n), (int)(1 + rand() % n)};
            sat.addClause(tmp);
        }
        ll ret = sat.solve();
        if (ret == 0) {
