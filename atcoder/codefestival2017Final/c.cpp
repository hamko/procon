...
int main(void) {
    ll n; cin >> n; 
    vll a; a.pb(0); rep(i, n) { ll tmp; cin >> tmp; a.pb(tmp); }
    n = n + 1;
    ...
    rep(d, 30) {
        ...
        rep(i, n) repi(j, i+1, n) {
            if (diff(a[i], a[j]) < d) { vector<int> tmp = {-(int)(1 + i), -(int)(1 + j)}; sat.addClause(tmp); }
            if (diff(a[i], 24-a[j]) < d) { vector<int> tmp = {-(int)(1 + i), +(int)(1 + j)}; sat.addClause(tmp); }
            if (diff(24-a[i], a[j]) < d) { vector<int> tmp = {+(int)(1 + i), -(int)(1 + j)}; sat.addClause(tmp); }
            if (diff(24-a[i], 24-a[j]) < d) { vector<int> tmp = {+(int)(1 + i), +(int)(1 + j)}; sat.addClause(tmp); }
        }
        ll ret = sat.solve();
        if (ret == 0) {
            ans = d;
