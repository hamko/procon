...
int main(void) {
    ...
    /*
    {
        ll j = 1;
        repi(i, 1, 1+n) {
            cout << mt(i, j) << " " << dp[i][j % 2] << endl;
        }
    }
    */
    SegmentTreeMax<ll> seg(n+1); 
    repi(j, 2, k+1) {
        ll now = j % 2; 
        ll prev = (j + 1) % 2; 
        ll dl = 0, dr = 0;
        ...
        repi(i, 1, n+1) if (j <= i) {
            ...
            ll r = i;
            while (dr < r) {
                while (d.size() && d.back().fi <= dp[dr][prev]) {
                    d.pop_back();
                }
                d.push_back(P(dp[dr][prev], dr));
                dr++;
            }
            while (dl < l) {
...
//            cout << d << endl;
//            ll realx = -INF;
//            repi(y, l, r) cout << dp[y][prev] << " "; cout << endl;
//            repi(y, l, r) chmax(realx, dp[y][prev]);
            ll x = d.size() ? d[0].fi : -INF;
            /*
            if (realx != x) {
                cout << "HIT!!!!!!!!!!" << endl;
                cout << realx << " " << x << endl;
            ...
            */
//            cout << x << endl;
            chmax(dp[i][now], x + j * a[i - 1]);
        }
        /*
        repi(i, 1, 1+n) {
            cout << mt(i, j) << " " << dp[i][now] << endl;
        }
        */
    }
    ll ret = 0;
