...
public:
    ...
    void printSparse(void) {
        rep(i, size()) if (query(i) != op->T0) { cout << "(" << i << ", " << query(i) << ") "; } cout << endl;
    }
    void printAll(void) {
...
int main(void) {
    ...
    s.enableRangeUpdate(false); // これを切ると、op_lazy, resolve_lazyが定義不要になる。範囲更新しようとするとassertで落ちる。
