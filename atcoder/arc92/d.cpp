...
int main(void) {
    // 答えsとすると、
    // s[k] = count_{i, j} (2^k <= a_{i, k} + b_{j, k} < 2*2^k or 3*2^k<=a_{i,k}+b_{j,k}<4*2^k)
...
//        cout << k << endl;
        ...
        wavelet<int, 200010, 29> w(200010, b);
        ll c = 0;
        rep(i, n) {
            c += w.freq(0, n, 1ll*(1ll<<k)-a[i], 2ll*(1ll<<k)-a[i]);
            c += w.freq(0, n, 3ll*(1ll<<k)-a[i], 4ll*(1ll<<k)-a[i]);
        }
        ...
        sort(all(b));
        rep(i, n) {
            c += (ll)(lower_bound(all(b), 2ll*(1ll<<k)-a[i])-b.begin());
            c -= (ll)(lower_bound(all(b), 1ll*(1ll<<k)-a[i])-b.begin());
            c += (ll)(lower_bound(all(b), 4ll*(1ll<<k)-a[i])-b.begin());
            c -= (ll)(lower_bound(all(b), 3ll*(1ll<<k)-a[i])-b.begin());
        }
        */
        /*
        ll d = 0;
