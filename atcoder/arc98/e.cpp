...
ll dfs(ll n, ll k, ll q, vll a, ll M, ll m) {
//    cout << q << " " << a << " " << M << " " << m <<endl;
    if (q==0) return M-m;
    ll ret = INF;
    rep(i, a.size()-k+1) {
        ...
        rep(j, k) if (hoge == a[i+j]) {
            vll a_next;
            rep(h, a.size()) if (h != i + j) {
                a_next.pb(a[h]);
            }
            chmin(ret, dfs(n,k,q-1,a_next,max(M,a[i+j]),min(m,a[i+j])));
        }
    }
    return ret;
}

ll brutal(ll n, ll k, ll q, vll a) {
    return dfs(n,k,q,a,-INF,+INF);
}
...
//        cout << mid << endl;
        ...
        auto s = uf.getUnionList();
        vll p;
        for (auto& x : s) if (x.size() > 1ll || x[0] >= mid) {
            vll u;
            rep(i, (ll)x.size()) {
                u.pb(a[x[i]]);
            }
            sort(all(u));
            rep(i, max<ll>(0ll,(ll)x.size()-(ll)(k-1ll))) {
                p.pb(u[i]);
            }
        }
        sort(all(p));
