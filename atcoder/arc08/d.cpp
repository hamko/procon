...
public:
//    virtual P op(P a, P b) { return P(a.first * b.first, a.second + a.first * b.second); } 
    virtual P op(P a, P b) { return P(b.first * a.first, b.second + b.first * a.second); }
    virtual P op_lazy(P a, P b) { return P(0, 0); } // Range Add
    virtual P resolve_lazy(P d, P l, int nl, int nr) { return P(0, 0); } // 全部上がってるので、子によらず増える
    AssosiativeOperatorLinearFunction(void) { T0 = P(1, 0); }
};
...
int main(void) {
    ...
    ll n, m; cin >> n >> m;
    vector<pair<ll, P>> ary;
    rep(i, m) {
        ll p; ld a, b; cin >> p >> a >> b;
        ary.push_back(pair<ll, P>(p, P(a, b)));
    }
    ll si = compress(ary);

    auto ptr = new AssosiativeOperatorLinearFunction;
    SegmentTree<P> s(si, ptr);
//    SegmentTree<P> s(1<<14, ptr);
    ll i = 0;
    ld _m = 1, _M = 1;
    for (auto x : ary) {
        s.update(ary[i].first, x.second);
        P tmp = s.query(0, s.size());
        _m = min(_m, tmp.first + tmp.second);
        _M = max(_M, tmp.first + tmp.second);
        i++;
    }
    printf("%.40Lf\n", _m);
    printf("%.40Lf\n", _M);
