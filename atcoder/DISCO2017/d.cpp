...
ll score(void) {
    ll tate = 1, yoko = 1;
    rep(i, h) rep(j, w) {
        tate &= v[i][j] == v[h-i-1][j];
        yoko &= v[i][j] == v[i][w-j-1];
    }
    return (tate ? a : 0) + (yoko ? b : 0);
}
ll solve(void) {
    if (memo.count(v)) return memo[v];
    ll faf = 1;
    rep(i, h) rep(j, w) faf &= v[i][j] == '.';
    if (faf) return 0;

    ll ret = 0;
    rep(i, h) rep(j, w) if (v[i][j] == 'S') {
        v[i][j] = '.';
        chmax(ret, score() + solve());
        v[i][j] = 'S';
    }
    return memo[v] =ret;
}
...
//    cout << ans << endl;
    ...
    ll freq[5] = {};
    ll tate = 0, yoko = 0, naname = 0; // tate A, yoko B
    rep(i, h/2) rep(j, w/2) {
        vector<char> x = {v[i][j], v[h-i-1][j], v[i][w-j-1], v[h-i-1][w-j-1], };
        //        cout << x << endl;
        ll snum = count(all(x), (char)'S');
        //        cout << snum << endl;
        if (snum == 1) 
            freq[1]++;
        if (snum == 2 && x[0] != x[3]) 
        ...
        if (snum == 2 && x[0] == x[3]) 
            naname++;
        if (snum == 2 && x[0] == 'S' && x[1] == 'S') tate++;
        if (snum == 2 && x[2] == 'S' && x[3] == 'S') tate++;
        if (snum == 2 && x[0] == 'S' && x[2] == 'S') yoko++;
        if (snum == 2 && x[1] == 'S' && x[3] == 'S') yoko++;
        if (snum == 3) {
            freq[3]++;
        } else if (snum == 4) {
            freq[4]++;
    ...
    ll ret = 0;
//    cout << tate << " " << yoko << " " << naname<< endl;
//    cout << freq[3] << " " << freq[4] << endl;
    ret += max((tate+freq[3])*a, (yoko+freq[3])*b);
    ret += freq[4] * (a + b + max(a, b));
    ret += a + b;

    if (freq[1] == 0 && freq[2] == 0 && freq[3] == 0 && !naname && freq[4]) {
        ret -= a + b;
    } else if (freq[1] == 0 && tate && !yoko && !freq[3] && !naname) {
        ret -= a;
    } else if (freq[1] == 0 && !tate && yoko && !freq[3] && !naname) {
        ret -= b;
    }
