...
int main(void) {
    ...
    rep(i, n+1) rep(j, 6000) rep(h, 2) dp[i][j][h] = 0;
    dp[0][0][0] = 1;
    rep(i, n) rep(j, 5900) rep(h, 2) {
        rep(k, a[i]+1) {
            dp[i+1][j+k][h | (k == a[i])] += dp[i][j][h];
        }
    }

    /*
    rep(i, n+1) rep(j, 6000) rep(h, 2) if (dp[i][j][h].x) {
