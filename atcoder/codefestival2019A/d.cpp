...
int main(void) {
    ...
    vll l(n), r(n);
    rep(i, n) {
        l[i] = (ll)(lower_bound(all(a), a[i]+f-t) - a.begin());
        r[i] = (ll)(upper_bound(all(a), a[i]+f) - a.begin());
    }
    cout << l << endl;
    cout << r << endl;
    ...
    dp[0] = 1;
    repi(i, 1, n) {
        cout << mt(i, x[i], y[i]) << endl;
        repi(j, x[i], y[i]+1) {
            cout << mt(j, dp[j], (mint(2) ^ (l[j] - j - 1))) << "#" << endl;
            dp[i] += dp[j] * (mint(2) ^ (l[j] - j - 1));
        }
        cout << dp[i] << endl;
    }
    cout << dp[n] << endl;
