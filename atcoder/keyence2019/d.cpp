...
ll brutal(ll n, ll m, vll a, vll b) {
    vll id;
    ll ret = 0;
    rep(i, n*m) id.pb(i);
    do {
...
int main(void) {
    ...
    sort(all(a));
    sort(all(b));
    set<ll> as; rep(i, n) as.insert(a[i]);
    set<ll> bs; rep(i, m) bs.insert(b[i]);
//    cout << a << endl;
//    cout << b << endl;
    if (as.size() != n || bs.size() != m) {
        cout << 0 << endl;
        return 0;
    }

    vll freq(n*m); rep(i, n) rep(j, m) freq[min(a[i], b[j])]++;
    ll ret = 1;
    ll cnt = 0;
    for (ll i = n * m - 1; i >= 0; i--) {
        cnt += freq[i];
        if (as.count(i) && bs.count(i)) {
        } else if (!as.count(i) && !bs.count(i)) {
            (ret *= cnt) %= mo;
//            cout << i<< " " << cnt << endl;
        } else if (as.count(i)) {
            (ret *= m - (ll)(lower_bound(all(b), i) - b.begin())) %= mo;
//            cout << i << " " << (ll)(lower_bound(all(b), i) - b.begin()) << endl;
        } else {
            (ret *= n - (ll)(lower_bound(all(a), i) - a.begin())) %= mo;
//            cout << i << " " << (ll)(lower_bound(all(a), i) - a.begin()) << endl;
        }
        cnt--;
//        cout << ret << endl;
    }
