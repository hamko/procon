...
bool check(ll i, ll k, ll depth) {
    // 頂点iにコインを置いて、残りのk枚をdepth歩で行けるか
    vll cands;
    rep(i, n) if (!used[i]) {
        cands.pb(d[i]);
    }
    ...
    sort(all(cands));
//    cout << mt(i, k, depth) << " " << cands << endl;
    ll cm = 0, cM = 0;
    rep(i, k) {
        cm += cands[i];
        cM += cands[cands.size()-1-i];
    }
...
//    cout << used << endl;
    rep(i, m) {
        ...
        rep(j, n) if (!used[j]) {
            ...
            auto usedPrev = used;
            updateUsedNode(j); 
//            cout << j << " " << usedPrev << " " << used << endl;
            if (check(j, m - i - 1, depth - d[j])) {
                ok = 1;
                depth -= d[j];
                ret.pb(j);
//                cout << "OK!!!!!!" << " " << j << endl;
