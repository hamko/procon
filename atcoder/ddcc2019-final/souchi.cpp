...
#define INF (ll)1e18
#define mo  (ll)(1e9+7)
double timeDist(double xa, double ya, double xb, double yb) {
    double ret = sqrt((xa-xb)*(xa-xb)+(ya-yb)*(ya-yb));
    return ret;
}

P getNearPos(ll x, ll y) {
    do {
    } while (0
}
int main(void) {
    ...
    // 605079.1226618 mm^3
    //
    // 一分間では60 * 30 = 1800mlしかできないので、結局無理
    double AtoBtoE = 0;
    double BtoAtoE = 0;
    ...
    ll s = 1;
    ll d = 30;
    if (timeDist(xst, yst, xa, ya) > timeDist(xst, yst, xb, yb)) {
        cout << xb+(rand()%d) << "," << 500 << "," << 60 << "," << 60 << ",";
        cout << yb+(rand()%d) << "," << 500 << "," << 60 << "," << 60 << ",";
        cout << 0 << "," << s << "," << 0;
        cout << endl;

        cout << xa+(rand()%d) << "," << 500 << "," << 60 << "," << 60 << ",";
        cout << ya+(rand()%d) << "," << 500 << "," << 60 << "," << 60 << ",";
        cout << s << "," << 0 << "," << 0;
        cout << endl;
    } else {
        cout << xa+(rand()%d) << "," << 500 << "," << 60 << "," << 60 << ",";
