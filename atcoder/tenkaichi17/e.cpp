...
int main(void) {
    ...
    rep(i, n) rep(j, n) if (i < j) {
        ...
        double k =1.0/(a[i]*b[j] - a[j]*b[i]); 
        double xx = (b[j] * c[i] - b[i] * c[j]) * k;
        if (counter <= counter_max) {
            chmin(xmin, xx);
            chmax(xmax, xx);
        }
//        cout << xmin << " " << xmax <<"HOGE"<< endl;
        if (counter <= counter_max || (xmin <= xx && xx <= xmax)) {
            //        double yy = (-a[j] * c[i] + a[i] * c[j]) * k;
            posx.pb(xx);
            //        posy.pb(yy);
            //        cout << posx.back() << " " << posy.back() << endl;
//            cout << posx.back() << endl;
        } else if (xx < xmin) {
            smaller++;
        }
    ...
    cout << counter << endl;
    sort(all(posx));
    cout <<ldout<< posx[(n*(n-1)/2-1)/2-smaller] << endl;
    /*
    sort(all(posy));
    cout << posx << endl;
    cout << ldout << posx[(posx.size()-1)/2] << " " << posy[(posy.size()-1)/2] << endl;
    */
