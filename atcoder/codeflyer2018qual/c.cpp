...
int main(void) {
    ...
    vll mink(n);
    rep(i, n) mink[i] = lower_bound(all(a), a[i] + d + 1) - a.begin();
    vll minks(n+1);
    rep(i, n) minks[i+1] = minks[i] + mink[i];

    vll mini(n);
    rep(j, n) mini[j] = lower_bound(all(a), a[j] - d) - a.begin();


    ll warray[100010] = {};
    rep(i, n) warray[i] = mink[i];
    wavelet<ll, 100010, 32> w(100010, warray);

    ll ret = 0;
    rep(j, n) {
        ll m = w.freq(mini[j], j, 0, mink[j]);
        ret += m * mink[j] - (minks[mini[j]+m] - minks[mini[j]]);
    }
    cout << ret << endl;
