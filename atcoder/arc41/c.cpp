...
ll score(vll R, vll L) {
    ll ret = 0;
    vll rpos = {R.back(), L[0]-1};
    vll lpos = {R.back()+1, L[0]};
    rep(_, 2) {
//        cout << rpos[_] << " " << lpos[_] << endl;
        ll rdist = R.size() * rpos[_] - accumulate(all(R), 0ll) - R.size() * (R.size() - 1) / 2;
        ll ldist = accumulate(all(L), 0ll) - L.size() * lpos[_] - L.size() * (L.size() - 1) / 2;
        chmax(ret, rdist + ldist);
    }
    return ret;
}
