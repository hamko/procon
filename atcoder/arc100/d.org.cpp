...
ll n;
bool judge(vll& a, vll& as, ll k, ll l) {
    assert(l >= 2 && l < n-1);
    ...
    ll ret = INF;
    if (ok < l) { // あやしい
        chmin(ret, abs((as[ok]-as[0])-(as[l]-as[ok])));
    }
    if (ok-1>0) {
        chmin(ret, abs((as[ok-1]-as[0])-(as[l]-as[ok-1])));
    }
    return ret <= k;
}

bool judgeB(vll& a, vll& as, ll k, ll l) {
    assert(l >= 2 && l < n-1);
    ll ret = INF;
    repi(x, 1, l) {
        // a[0, x), a[x, l)
        chmin(ret, abs((as[x]-as[0])-(as[l]-as[x])));
    }
//    cout << ret << endl;
    return ret <= k;
}
