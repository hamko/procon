...
bool dfs(ll u, ll p, ll to, vll& path) {
    if (u == to) {
        path.pb(u);
        return true;
    }
...
void smax(ll u, ll p, ll depth, vll& s) {
    chmax(s[depth], g[u].size()-(p!=-1));
    for (auto x : g[u]) if (x != p) {
        smax(x, u, depth+1, s);
    }
}
...
int main(void) {
    ...
    if (path.size() % 2 == 0) {
        ...
        vll s(path.size()/2+1);
        ll x = path[path.size()/2];
        ll ret = g[x].size();
        cout << ret << endl;
        smax(x, -1, 0, s);
        cout << s << endl;
        repi(i, 1, path.size() / 2) {
            ret *= s[i];
        }
        cout << ret << endl;
    }
