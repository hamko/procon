...
int main(void) {
    ...
    rep(l, n+1) {
        rep(i, n - l + 1) {
            if (l <= 2) continue;
            if (l == 3) {
                dp[l][i] = (s[i] == 'i' && s[i+1] == 'w' && s[i+2] == 'i');
                continue;
            }

            rep(left_l, l) {
                ll right_l = l - left_l;
                chmax(dp[l][i], dp[left_l][i] + dp[right_l][i+left_l]);
            }
            if (l % 3 != 0 || s[i] == 'w' || s[i+l-1] == 'w') continue;
            repi(j, i, i+l) if (s[j] == 'w') {
                if ((j-(i+1)) != 3 * dp[j-(i+1)][i+1]) continue;
                if (((i+l-1)-(j+1)) != 3 * dp[(i+l-1)-(j+1)][j+1]) continue;
                chmax(dp[l][i], dp[j-(i+1)][i+1] + dp[(i+l-1)-(j+1)][j+1] + 1);
            }
        }
    }
    cout << dp[n][0] << endl;
