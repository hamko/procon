...
int main(void) {
    ...
    cout << c << endl;
    */
    rep(i, 2) rep(j, C+1) rep(h, W+1) rep(k, 2) dp[i][j][h][k] = -INF;
    dp[0][0][0][0] = 0;

    // 配るDP
    rep(i, n) {
        ll curr = i % 2; ll next = (curr + 1) % 2;
        rep(j, C+1) rep(h, W+1) rep(k, 2) {
            dp[next][j][h][k] = -INF;
        }
        rep(j, C+1) rep(h, W+1) rep(k, 2) if (dp[curr][j][h][k] != -INF) {
            ll& val = dp[curr][j][h][k];
            if (k && h + w[i] <= W) {
                if (i > 0 && c[i] == c[i-1]) {
                    chmax(dp[next][j][h+w[i]][1], val + v[i]);
                } else {
                    chmax(dp[next][j+1][h+w[i]][1], val + v[i]);
                }
            }
            ...
            if (i > 0 && c[i] == c[i-1]) {
                chmax(dp[next][j][h][k], val);
            }
            if (i == 0 || c[i] != c[i-1]) {
                chmax(dp[next][j][h][0], val);
            }
        }
        /*
        rep(j, C+1) rep(h, W+1) rep(k, 2) if (dp[next][j][h][k] != -INF) {
            cout << i+1 << " " << j << " " << h << " " << k << " : " << dp[next][j][h][k] << endl;
        }
        */
    }

    ll ret = 0;
    ll i = n % 2; rep(j, C+1) rep(h, W+1) rep(k, 2) {
        chmax(ret, dp[i][j][h][k]);
    }
    cout << ret << endl;
    return 0;
}
