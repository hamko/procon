...
int main(void) {
    ll n, k; cin >> n >> k;
//    fenwick_tree<ll> dp(1000010);
    fenwick_tree<ll> dp(n+1);
    dp.update(1, 0);
    repi(i, 2, n+1) {
        dp.update(i, dp.query(i-1) - dp.query(max<ll>(0, i-k-1)) + (i - 1 < k));
    }
//    dp.print();
    cout << ((dp.query(n-1) - dp.query(n-k)) % mo + mo) % mo<< endl;
