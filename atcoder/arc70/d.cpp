...
ll f(vll& a) {
    ll n = a.size();
    // dp[i][j] = i個見て和がkになるような部分集合の場合の数
    memset(dp, 0, sizeof(dp));
    dp[0][0] = 1;
    rep(i, n) {
        rep(j, k+1) {
           dp[i+1][j] += dp[i][j]; if (dp[i+1][j] >= mo) dp[i+1][j] -= mo;
           ll h = min(k, j+a[i]);
           dp[i+1][h] += dp[i][j]; if (dp[i+1][h] >= mo) dp[i+1][h] -= mo;
        }
    }
    return dp[n][k];
}
...
ll full = 0;
bool check(ll q) {
    vll a; rep(i, N) if (q != i) a.pb(b[i]);
    return f(a) * 2 % mo != full;
}
int main(void) {
    cin >> N >> k;
    b.resize(N); cin >> b;
