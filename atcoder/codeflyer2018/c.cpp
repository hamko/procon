...
int main(void) {
    ...
    {
        ll n = 6;
        ll a[] = {200000, 200001, 200002, 200003, 200002, 200001,};
        wavelet<ll, 10000/*len*/, 20/*[0, 31)*/> w(n, a);
        cout << w.freq(2, 6, 200001, 200002) <<"test"<< endl;
    }
#endif 
    string s; 
    cin >> s;
...
//            cout << ok << " " << ng << endl;
            ll mid = (ok + ng) / 2;
            (w.freq(i, mid+1, 0, a[i]) > 0 ? ok : ng) = mid;
        }
        // okがマイナスポイント
//        cout << i << " " << "OK" << " " << ok << endl;
//        cout << i << " " << mt(i+1, ok, a[i], a[i]+1)<< endl;
        ll tmp = w.freq(i+1, ok, a[i], a[i]+1);
//        cout << tmp << "$NUM"  << endl;
        ret += tmp;
    }
    cout << ret << endl;
