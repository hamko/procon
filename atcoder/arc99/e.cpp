...
vll used;
vvll g;
P dfs(ll v, ll p, ll f) {
//    cout << mt(v, p, f) << " " << used[v] << endl;
    if ((used[v] == 1 && f == 0) || (used[v] == 0 && f == 1)) {
        cout << -1 << endl;
        exit(0);
    }
    if (used[v] != -1) return P(0, 0);
    used[v] = f;
    P ret;
    if (f == 0) 
        ret = P(1, 0);
...
//    cout << c << endl;
    ...
    n = c.size();
    bool dp[n+1][1000];
    rep(i, n+1) rep(j, 1000) dp[i][j] = 0;
    dp[0][0] = 1;
    rep(i, n) rep(j, 1000) if (dp[i][j]) {
        dp[i+1][j+c[i].fi] = 1;
        dp[i+1][j+c[i].se] = 1;
    }
    ll ret = INF;
    rep(j, 1000) if (dp[n][j]) {
        ll h = num_v - j;
        chmin(ret, j * (j - 1ll) / 2ll + h * (h - 1ll) / 2ll);
    }
    cout << ret << endl;
