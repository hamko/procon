...
int main(void) {
    ll n, m; cin >> n >> m;
    vector<vector<P>> freq(n+1);
    rep(i, m) {
        ll l, r, x; cin >> l >> r >> x;
        freq[r].pb(P(l, x));
    }
    dp[0][0][0] = 1;
    rep(r, n+1) rep(g, n+1) rep(b, n+1) if (dp[r][g][b].x) {
        ll left = max({r, g, b});
        if (left == n) continue;
        ll faf;

        faf = 1;
        for (auto x : freq[left+1]) 
            faf &= ((x.fi <= left + 1) + (x.fi <= g) + (x.fi <= b)) == x.se;
        if (faf) dp[left+1][g][b] += dp[r][g][b];

        faf = 1;
        for (auto x : freq[left+1]) 
            faf &= ((x.fi <= r) + (x.fi <= left + 1) + (x.fi <= b)) == x.se;
        if (faf) dp[r][left+1][b] += dp[r][g][b];

        faf = 1;
        for (auto x : freq[left+1]) 
            faf &= ((x.fi <= r) + (x.fi <= g) + (x.fi <= left + 1)) == x.se;
        if (faf) dp[r][g][left+1] += dp[r][g][b];
    }
    mint ret = 0;
    rep(r, n+1) rep(g, n+1) rep(b, n+1) if (max({r, g, b} == n) 
        ret += dp[r][g][b];
    cout << ret << endl;
