...
int main(void) {
    ...
    ll dp[b.size()+1][5];
    rep(i, b.size()+1) rep(j, 5) dp[i][j] = INF;
    dp[0][0] = 0;
    rep(i, b.size()) {
        rep(j, 5) if (dp[i][j] != -INF) {
            ll even = b[i] % 2;
            ll odd = (b[i] + 1ll) % 2;
            if (b[i] == 0) {
                even = 2;
            ...
            repi(h, j, 5) {
                if (h == 0) {
                    chmin(dp[i+1][h], dp[i][j] + b[i]); // iを取らない
                } else if (h == 1) {
                    chmin(dp[i+1][h], dp[i][j] + even); // iを偶数として取る
                } else if (h == 2) {
                    chmin(dp[i+1][h], dp[i][j] + odd); // iを奇数としてとる
                } else if (h == 3) {
                    chmin(dp[i+1][h], dp[i][j] + even); // iを偶数として取る
                } else {
                    chmin(dp[i+1][h], dp[i][j] + b[i]); // iを取らない（とれない）
                }
            }
        }
    }
    ll ret = INF;
    rep(j, 5) chmin(ret, dp[b.size()][j]);
    cout << ret << endl;
