...
double nCr(ll n, ll r) {
    if (ncr.count(P(n, r))) return ncr[P(n, r)];
    double ret = 1;
    rep(i, r) {
        ret *= n - i;
        ret /= i + 1;
    }
    return ncr[P(n, r)] = ret;
}
ll n, k; 
double memo[1010];
double prob(ll i) {
    ...
    double ret = 0;
    repi(x, i, n-2+1) {
        ret += (double)nCr(x, i) / (double)nCr(n-2, i);
    }
    ret /= n - 1;
    return memo[i] = ret;
    */
...
double f(ll i, ll j) {
    ...
    if (j == k) return dp[i][j] = 0;
    if (n-i <= k-j) return dp[i][j] = 1;
    if (dp[i][j] != -INF) return dp[i][j];

    double o = 1.l / (n - i);
    double x = (1.l - o) * (1. - prob(i));
    double y = (1.l - o) * prob(i);
//    cout << i <<" "<<j<< " : " <<  o << " " << y << " " << x << endl;

    double ret = -INF;
    chmax(ret, x * f(i+1, j) + y * f(i+1, j+1) + o * 1.l);
    chmax(ret, x * f(i+1, j) + y * f(i+1, j) + o * 0.l);
    return dp[i][j] = ret;
}
...
//    cout << endl;
    ...
    rep(i, 1010) rep(j, 1010) if (dp[i][j] != -INF) {
//        cout << i << " " << j << " : " << dp[i][j] << endl;
    }
     
    cout << ldout << min(ret, 1.l) << endl;
    return 0;
}
