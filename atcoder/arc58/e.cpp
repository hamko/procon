...
#define mo  (ll)(1e9+7)
ll n, x, y, z; 
bool subset(ll a, ll b) {
    return (b & a) == b;
}
int main(void) {
    cin >> n >> x >> y >> z;
    // dp[i][mask] = ちょうどi個先頭から見て、
    // 末尾がmaskであるような場合に、
    // 575が含まれていない場合の数
    vector<vector<ll>> dp(n+1, vll(1ll<<(x+y+z+1)));
    dp[0][0] = 1;

    ll ng = (1ll<<(z-1))|(1<<(z+y-1))|(1ll<<(z+y+x-1));

    // 配るDP
    rep(i, n) {
        rep(mask, dp[i].size()) if (dp[i][mask]) {
            repi(k, 1, 11) {
                ll next_mask = mask;
                next_mask <<= 1;
                next_mask += 1;
                next_mask <<= k - 1;
                next_mask &= ((1ll<<(x+y+z))-1);
                if (!subset(next_mask, ng)) {
                    dp[i+1][next_mask] += dp[i][mask];
                    if (dp[i+1][next_mask] > mo) dp[i+1][next_mask] -= mo;
                }
            }
        }
    }
