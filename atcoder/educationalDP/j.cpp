...
bool reachable[305][305][305];
void dfs(ll i, ll j, ll k) {
    if (reachable[i][j][k]) return;
    reachable[i][j][k] = 1;
    cout << mt(i, j, k) << "#"<< endl;
    if (j) dfs(i+1, j-1, k);
    if (i) dfs(i-1, j, k);
...
int main(void) {
        ...
        for (ll j = n; j >= 0; j--) {
            for (ll i = n; i >= 0; i--) {
                ...
                if (s) {
                    dp[i][j][k] = 1;
                    // (i-1, j+1, k) s からjを選択
                    if (0 <= i-1 && reachable[i-1][j+1][k]) {
                        cout << "j"<<endl;
                        dp[i][j][k] += dp[i-1][j+1][k] * (double)(j + 1) / s;
                    }
                    // (i+1, j, k)   s + 1 からiを選択
                    if (reachable[i+1][j][k]) {
                        cout <<mt(i, j, k) << " " << dp[i+1][j][k] << " " << i + 1 << " " << s << " " << (double)n / s<< "i"<<endl;
                        dp[i][j][k] += dp[i+1][j][k] * (double)(i + 1) / (s + 1);
                    }
                    // (i, j-1, k+1) s からkを選択
                    if (0<=j-1 && reachable[k+1][j-1][i]) {
                        cout << mt(i, j, k) << " " << (double)n / s<<  "k"<<endl;
                        dp[i][j][k] += dp[i][j-1][k+1] * (k + 1) / s;
                    }
                    dp[i][j][k] *= (double)n / s;
                } else {
                    dp[0][0][0] = dp[1][0][0] + 1;
    ...
    for (ll i = n; i >= 0; i--) {
        for (ll j = n; j >= 0; j--) {
            for (ll k = n; k >= 0; k--) if (dp[i][j][k]) {
                cout << mt(i, j, k) << " " << dp[i][j][k] << endl;
            }
        }
    }
    cout << dp[0][0][0] << endl;
