...
public:
    ...
    vll val, lazy; // val.size() = lazy.size() = 2 * n
    ll max_n = 1000000;

    ll n;
    ll e = 0;
    SegmentTreeProd (ll n_) {
        n = 1;
        while (!(n_ < n)) {
            n *= 2;
        }
    ...
    void push(ll k, ll l, ll r) {
        ...
        if (r - l == 1) {
            ...
            if (lazy[k] == 0) {
                ...
                if (lazy[2*k+0] == INF) { 
                    lazy[2*k+0] = lazy[k] / 2ll;
                } else {
                    lazy[2*k+0] += lazy[k] / 2ll;
                }
                if (lazy[2*k+1] == INF) { 
                    lazy[2*k+1] = lazy[k] / 2ll;
                } else {
                    lazy[2*k+1] += lazy[k] / 2ll;
                }
            }
    ...
    // after the function is done, lazy[k] = nothing and val[k] should be the correct data
    ll get(ll a, ll b, ll k, ll l, ll r) { // k is a node id, representing [l, r)
        push(k, l, r);
        if (b <= l || r <= a) return e; // not relevant, unit value [a, b) [l, r) or [l, r) [a, b)
        if (a <= l && r <= b) return val[k]; // selected
        ll ret = e;
        ret += get(a, b, 2*k+0, l, (l+r)/2);
        ret += get(a, b, 2*k+1, (l+r)/2, r);
        return ret;
    }
    ll get(ll a, ll b) { // [a, b) is a query interval
