...
ll dfs(ll n, ll x) {
    if (n == 0) return 1ll;
    ll prevAll = ((1ll << (n + 1)) - 3ll);
    ll prevPatty = ((1ll << (n + 0)) - 1ll);
//    cout << mt(n, x) << " " << prevAll << " " << prevPatty << endl;
    if (x == 1ll) {
        return 0;
    } else if (x <= 1ll + prevAll) {
        return dfs(n-1, x-1);
    } else if (x <= 1ll + prevAll + 1ll) {
        return prevPatty + 1ll;
    } else {
        return prevPatty + 1ll + dfs(n-1, x-2-prevAll);
    }
}
