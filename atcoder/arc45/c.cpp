...
int main(void) {
    ...
    map<P, ll> c;
    rep(i, n-1) {
        ll u, v, cost; cin >> u >> v >> cost; u--, v--;
        g[u].pb(v);
        g[v].pb(u);
        c[P(u, v)] = c[P(v, u)] = cost;

    }
    vll s(n);
    map<ll, ll> counter;
    function<void(ll, ll)> f = [&](ll v, ll p) -> void {
        for (ll nextv : g[v]) if (nextv != p) {
            s[nextv] = s[v] ^ c[P(v, nextv)];
            counter[s[nextv]]++;
            f(nextv, v);
        }
    };
    ...
    ll ret = 0;
    if (x) {
        for (auto y : counter) if (counter.count(x ^ y.fi)) {
            ret += y.se * counter[x ^ y.fi];
        }
        ret /= 2;
        ret += counter[x];
    } else {
