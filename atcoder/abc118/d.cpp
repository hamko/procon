...
int main(void) {
    ll n, m; cin >> n >> m;
    vll num = {0, 2, 5, 5, 4, 5, 6, 3, 7, 6};
    map<ll, ll> can_rev;
    map<ll, ll> can;
    ll minnum = INF;
    rep(i, m) {
        ll b; cin >> b;
        chmax(can_rev[num[b]], b);
        chmin(minnum, num[b]);
    }
    for (auto x : can_rev) {
        can[x.se] = x.fi;
    ...
    const ll V = 20000;
    vvll g(V);
    rep(i, V) { for (auto x : can) if (i + x.fi < V) 
        g[i + x.se].pb(i);
    ...
    function<ll(ll)> dfs = [&](ll i) {
        ...
        for (auto x : can) if (i-x.se >= 0 && dfs(i-x.se) != -INF) {
            if (dp[i] <= dfs(i-x.se)+1) {
                dp[i] = dfs(i-x.se)+1;
                id[i] = i-x.se;
            }
        }
