...
int main(void) {
    ...
    cout << ret << endl;
    vll am(n+1, INF), bm(n+1, INF);
    vll aM(n+1, -INF), bM(n+1, -INF);
    rep(i, n) {
        bm[i+1] = min(bm[i], ab[i][1]);
        bM[i+1] = max(bM[i], ab[i][1]);
        am[i+1] = min(am[i], ab[n-1-i][0]);
        aM[i+1] = max(aM[i], ab[n-1-i][0]);
    }
    cout << am << endl;
    cout << aM << endl;
    cout << bm << endl;
    cout << bM << endl;
    rep(k, n+1) {
        cout <<aM[k] << " " <<  bM[n-k] << " " << am[k] << " " << bm[n-k] << endl;
        chmin(ret, (rmax-rmin) * (max(aM[k], bM[n-k]) - min(am[k], bm[n-k])));
    }
    cout << ret << endl;
    return 0;
}
