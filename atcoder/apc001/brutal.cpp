...
vll a;
ll ret = INF;
void dfs(ll m, ll used, ll cost) {
    if (m == n-1) {
        chmin(ret, cost);
    }
    UnionFind uf(n);
    rep(i, n) for (auto x : g[i]) uf.unite(i, x);
    ll ret = INF;
    rep(i, n) repi(j, i+1, n) if (uf.find(i, j) == 0) if ((used & (1ll << i)) == 0) if ((used & (1ll << j)) == 0) {
        g[i].pb(j);
        g[j].pb(i);
        used |= 1ll << i;
        used |= 1ll << j;
        dfs(m+1, used, cost + a[i] + a[j]);
        g[i].pop_back();
        g[j].pop_back();
        used = ~((~used) | (1ll << i));
        used = ~((~used) | (1ll << j));
