...
int main(void) {
    ...
    vector<priority_queue<ll, vll, greater<ll>>> q(100010);
    priority_queue<P, vector<P>, greater<P>> memo;
    priority_queue<P, vector<P>, greater<P>> memo2;
    vector<vector<int>> tmp = uf.getUnionList();
    ll cyc = 0;
    for (auto&& s : tmp) {
        for (auto&& x : s) {
            q[cyc].push(a[x]);
        ...
        if (q[cyc].size() > 1) 
            memo2.push(P(q[cyc].top(), cyc));
        else 
            memo.push(P(q[cyc].top(), cyc));
        cyc++;
    }
    ...
    while (rem) {
        ...
        q[qi1].pop(), q[qi2].pop();
        assert(qi1!=qi2);
        if (q[qi1].size() < q[qi2].size()) {
            swap(qi1, qi2);
        }
        while (q[qi2].size()) {
            q[qi1].push(q[qi2].top()); q[qi2].pop();
        }
        if (q[qi1].size() > 1) 
            memo2.push(P(q[qi1].top(), qi1));
        else 
            memo.push(P(q[qi1].top(), qi1));
