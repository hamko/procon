...
ll n, m, a, b; 
ll node(ll i, ll j, ll mode) {
    return mode*n*m+(i*m+j);
}
int main(void) {
    cin >> n >> m >> a >> b;
    /*
       rep(i, n) rep(j, m) rep(mode, 2){
       cout << mt(i,j,mode) << " " << node(i, j, mode) << endl;
       }
       */
    SAT sat(2*n*m);
    rep(i, n) {
        rep(j, m) {
            sat.addClause(-(1+node(i,j,0)), -(1+node(i,j,1)));
            if (i+1<n) sat.addClause(-(1+node(i,j,0)), -(1+node(i+1,j,0)));
            if (i+1<n) sat.addClause(-(1+node(i,j,0)), -(1+node(i+1,j,1)));
            if (j+1<m) sat.addClause(-(1+node(i,j,1)), -(1+node(i,j+1,0)));
            if (j+1<m) { 
                sat.addClause(-(1+node(i,j,1)), -(1+node(i,j+1,1)));
                cout << mt(-(1+node(i,j,1)), -(1+node(i,j+1,1))) << endl;;
            }
        }
    }
    rep(j, m) sat.addClause(-(1+node(n-1,j,0)), -(1+node(n-1,j,0)));
    rep(i, n) sat.addClause(-(1+node(i,m-1,1)), -(1+node(i,m-1,1)));
    bool ret = sat.isSAT();
    if (ret) {
        cout << "YES"<< endl;
        ...
        rep(mode, 2)  {
            rep(i, n) {
                rep(j, m) {
                    cout << ans[node(i,j,mode)];
                    /*
                       if (ans[node(i, j, mode)]) {
                       if (mode) {
                       b[i][j] = '<';
                       } else {
                       b[i][j] = '^';
