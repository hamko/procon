...
int main(void) {
    ...
    rep(i, n) {
        g ^= a[i];
//        cout << (a[i]^a[i]-1) << " " << __builtin_ctzll((a[i]^(a[i]-1))+1) << endl;
        b[__builtin_ctzll((a[i]^(a[i]-1))+1)-1] = 1;
    }
//    cout << bitset<30>(g) << endl;

    ll ret = 0;
    for (ll i = 60; i >= 0; i--) if (g & (1ll << i)) {
        if (b[i]) {
            ret++;
            g ^= (1ll << (i+1))-1;
//            cout << bitset<30>((1ll << (i+1))-1) << endl;
//            cout << bitset<30>(g) << endl;
        } else {
            cout << -1 << endl;
            return 0;
