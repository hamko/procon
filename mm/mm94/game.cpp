...
public:
    ...
    // 1探索ステップで、正しく、かつ同じu, vを出さないことまで保証する
    // insertは関数外で行う。
    ...
    tuple<int, int> proposeUVCorderExternal(int maxi, int maxj) {
        ...
        int u = cand[rand()%4];
        if (rand() % 2) {
            if (peval.li < n - peval.ri) {
                return mt(u, min<int>(peval.ri + (rand() % (n-peval.ri)), n-1));
            } else {
                return mt(u, rand() % (peval.li + 1));
            }
        } else { 
            if (peval.lj < n - peval.rj) {
                return mt(u, min<int>(peval.rj + (rand() % (n-peval.rj)), n-1));
            } else {
                return mt(u, rand() % (peval.lj + 1));
            }
        }
        */
