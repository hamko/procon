...
class ConnectedComponent {
    ...
    vector<int> permute(vector<int> m) {
        ...
        auto info = evaluate(p);
        data initial = {conv(m), p, info, getPoint(info)};

        vector<data> next_q = {initial}; // TODO 初期値をランダムに複数に！
        /* n = 50
        ll beam_width = 20; // 3だとsample1で全然多様性が確保できなかった
        double annealing_first = 0.95;
        double annealing_end = 0.95;
        double annealing_period = 10000;
        ll num_of_random_transite = 10;
        ll num_of_elite = 2;
        */
        ll beam_width = 20; // 3だとsample1で全然多様性が確保できなかった
        double annealing_first = 0.95;
        double annealing_end = 0.95;
        double annealing_period = 100;
        ll num_of_random_transite = 10;
        ll num_of_elite = 2;
//        while (next_q.size()) {
        ll cyc = 0;
        while (1) { cyc++; 
            double prob = annealing_first + (annealing_end - annealing_first) * min<double>(1, 1.0*cyc/annealing_period);

            vector<data> tmp;
            sort(all(next_q));

            cout << cyc << " " << sec() << " " << getPoint(evaluate(next_q[0].p)) << endl;
            if (cyc % 1 == 0 && sec() > TL) { // cyc % 10 == 0は、あまりいっぱいsec()を呼ばないため
                break;
            }

            // Elite
            rep(i, min<ll>(num_of_elite, next_q.size())) 
                tmp.pb(next_q[i]);

            // TODO ここで点数が完全に同じものは除外しておいたほうが良い
            ...
            */
            rep(i, min<ll>(beam_width, next_q.size())) {
                rep(_, num_of_random_transite) {
                    ll real_i = i;
                    if (i >= num_of_elite && (double)rand()/RAND_MAX > prob) {
                        real_i = (ll)((double)rand()/RAND_MAX*next_q.size());
                    }
                    tmp.pb(randomTransite(next_q[real_i]));
                }
            }
            swap(tmp, next_q);
        }
...
//        print(next_q[0].p);
        cout << getPoint(evaluate(next_q[0].p)) << endl;
        vector<int> ret; rep(i, n) ret.pb(next_q[0].p[i]);
        return ret;
    }   
};  
