...
public:
    ...
    void printBoard(void) { rep(i, n) { rep(j, n) if (xy(i, j) >= 0) cout << (int)xy(i, j); else cout << (char)('A'-xy(i, j)); cout << endl; } }
    void printUsed(void) { rep(i, n) { rep(j, n) if (!used[i*n+j]) cout << "_"; else if (xy(i, j) >= 0) cout << (int)xy(i, j); else cout << (char)('A'-xy(i, j)); cout << endl; } }
 ...
 //           cout << logging << " " << scores << endl;
            ...
            //            cout << "->" << endl;
            //            printUsed();
            //            cout << pred << " " << next_score - score << "#pred" << endl;
...
//            cout << counter << endl;
//            cout << maxij << endl;
            if (logging.size() && counter > 100) {
//                cout << "#back" << endl;
                counter = 0;

                P x = logging.back(); logging.pop_back();
                int u_last, v_last; u_last = x.fi, v_last=x.se;

                P maxij_element = maxij.back(); maxij.pop_back();
                maxi = maxij_element.fi, maxj = maxij_element.se;

                score = scores.back();
                scores.pop_back();

                swap_rc(u_last, v_last);
//                cout << "#back" << endl;
//                printBoard();
//                cout << maxi <<" " << maxj << "$backed maxij"<< endl;
                continue;
            }
            bool f = getForceNext(next_score - score) ;
            f = 0;
//            cout << score << " " << next_score << endl;
            if (f || score < next_score - 1e-4) {
                logging.pb(P(u, v));
                maxij.pb(P(maxi, maxj));
//                cout << maxi <<" " << maxj << "$pushed maxij"<< endl;
                scores.pb(score);
//                cout << counter << " "<< f<<"#counter"<< endl;
                cout << cyc << " " << t << " " << score << " " << max_score << " " << max_num<< "#score" << endl;
                counter= 0;
                score = next_score;
...
//                cout << cyc << " " << t << " " << score << " " << max_score << " " << max_num<< "#score" << endl;
        ...
        cout << counter << "#counter remaining" << endl;
