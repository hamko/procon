...
//                cout << p << endl;
            ...
            /*
            rep(j, n) {
                repi(i, max<ll>(0ll, u-1), min<ll>(n-1, u+1+1)) if (!used[i*n+j] && m[i*n+j]) f(i, j);
                repi(i, max<ll>(0ll, v-1), min<ll>(n-1, v+1+1)) if (!used[i*n+j] && m[i*n+j]) f(i, j); // これと
            }
            rep(i, n) {
                repi(j, max<ll>(0ll, u-1), min<ll>(n-1, u+1+1)) if (!used[i*n+j] && m[i*n+j]) f(i, j); // これがなくても普通にスコアは上がる。しかも2倍速に…
                repi(j, max<ll>(0ll, v-1), min<ll>(n-1, v+1+1)) if (!used[i*n+j] && m[i*n+j]) f(i, j);
            }
            */
            // +0 Greedy
            /*
            rep(j, n) {
                if (!used[u*n+j] && m[u*n+j]) f(u, j);
                if (!used[v*n+j] && m[v*n+j]) f(v, j);
            }
            rep(i, n) {
                if (!used[i*n+u] && m[i*n+u]) f(i, u);
                if (!used[i*n+v] && m[i*n+v]) f(i, v);
            }
            */
