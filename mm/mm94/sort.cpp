...
//        rep(i, n) rep(j, n) sums[i].fi += !!m[i*n+j] + !!m[j*n+i], sums[i].se = i;
        ...
        rep(i, n) rep(j, n) m[i*n+j] = b_org[p[i]][p[j]];
    }
    void init_maxij(void) {
        maxi = n/2, maxj = n/2+1;
        int x = INF;
        // なるべく内側＝連結成分が大きいものを使う
        rep(i, n) rep(j, n) if (i != j && m[i*n+j]>0){
    ...
    pair<double, int> evaluate(int i, int j) {
        ...
        used[i*n+j] = 1;
        while (q.size()) {
            int curr_ij = q.front(); q.pop();
            rep(d, 4) {
                int ni = curr_ij/n+di[d], nj = curr_ij%n+dj[d];
                int next_ij = curr_ij+di[d]*n+dj[d];
    ...
    vector<int> permute(vector<int> m_) {
        ...
        while (1) { cyc++; 
            ...
            for (int d : dirs) {
                ...
                if (tmpi >= 0 && tmpi < n && tmpj >= 0 && tmpj < n && tmpi != tmpj && m[tmpi*n+tmpj]) 
                    maxi = tmpi, maxj = tmpj;
            }
