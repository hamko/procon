...
//            cout << "BRUTAL" <<endl;
    ...
    double calcRowNum(vector<int>& m, ll u) { ll ret = 0; rep(j, n) ret += !!m[u*n+j]; return ret; }
    double calcColNum(vector<int>& m, ll u) { ll ret = 0; rep(i, n) ret += !!m[i*n+u]; return ret; }
    double calcRowVal(vector<int>& m, ll u) { ll ret = 0; rep(j, n) ret += m[u*n+j]; return ret; }
    double calcColVal(vector<int>& m, ll u) { ll ret = 0; rep(i, n) ret += m[i*n+u]; return ret; }
public:
    ...
    vector<int> permute(vector<int> m) {
        ...
        vll cols(n), rows(n);
        rep(i, n) rep(j, n) cols[i] += m[i*n+j] > 0;
        rep(j, n) rep(i, n) rows[j] += m[i*n+j] > 0;
...
//        double startTemp = 0.1, endTemp = 0.1; 
        double T = TL;
        ll R = 10000;
        double t = 0;
        while (1) { cyc++; 
...
//                cout << "#DFS" <<endl;
            ...
            /*
            if (score < next_score + 1e-4) {
                cout << u << " " << v << " " << calcColNum(m, u) << " " << calcColNum(m, v) << " "  << calcRowNum(m, u) << " " << calcRowNum(m, v) << " " 
                    << calcColVal(m, u) << " " << calcColVal(m, v) << " "  << calcRowVal(m, u) << " " << calcRowVal(m, v) 
                    << "#feature_ok"<<endl;
            } else {
                cout << u << " " << v << " " << calcColNum(m, u) << " " << calcColNum(m, v) << " "  << calcRowNum(m, u) << " " << calcRowNum(m, v) << " " 
                    << calcColVal(m, u) << " " << calcColVal(m, v) << " "  << calcRowVal(m, u) << " " << calcRowVal(m, v) 
