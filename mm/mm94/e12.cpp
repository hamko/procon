...
public:
    ...
    int maxi, maxj;         // ä»ã®ç¤é¢ã§ã®æå¤§é£çµæåã®åº§æ¨ã®ä¸ä¾
    double score = -INF;    // ä»ã®ã«ã¼ãã®ã¹ã³ã¢
    int num = -1;           // ä»ã®ã«ã¼ãã§ã®æå¤§é£çµæåã®ãµã¤ãº

    /****** æ¸©åº¦ ******/
    double max_score = -INF; // æé«ã¹ã³ã¢
    int max_num = -1;       // æé«ã¹ã³ã¢æã®
    ...
    vector<int> permute(vector<int> m_) {
        ...
        unordered_set<P> memo; // 1ã»ããã®æ¢ç´¢ã§ããã§ã«æ¢ç´¢ãã(u, v)ãã¢éå
        map<double, P> scores; // 1ã»ããã®æ¢ç´¢ã§å¾ãã¹ã³ã¢ã¨ãã¢ã®è¾æ¸
        while (1) { cyc++; 
//            cout << "#########" << endl;
...
//                    cout << u << " " << v << " : " << hashBoard() << "#hit" << endl; // çµæ§ç¡é§ãªæ¢ç´¢ãã¦ã
                }
            }
